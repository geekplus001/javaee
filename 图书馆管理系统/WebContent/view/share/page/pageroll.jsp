...
<body>${param.action}
	...
	<form action="${param.action}" name="pageRollForm" method="post">
		...
		共${requestScope.pageRoll.totalCount}条记录| <input type="hidden"
		name="currPage" value="${requestScope.pageRoll.currPage}" /> <input
		type="hidden" name="pageCount"
		value="${requestScope.pageRoll.pageCount}" /> <input type="button"
		value="首页" onclick="firstPage()" /> <input type="button" value="上一页"
		onclick="prevPage()" /> <input type="button" value="下一页"
		onclick="nextPage()" /> <input type="button" value="尾页"
		onclick="lastPage()" /> <select name="goto_page"
		onchange="gotoPage()">
